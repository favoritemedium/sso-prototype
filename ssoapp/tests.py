...
class VerifyEmailTestCase(TestCase):
    ...
    def test_create(self):
        ...
        email1 = VerifyEmail.verify_token(token)
        self.assertEqual(email, email1)

    def test_not_found(self):
        email = fake.email()
        token = VerifyEmail.get_token(email)
        email1 = VerifyEmail.verify_token('adifferenttoken')
        self.assertIsNone(email1)

    def test_find_correct_email(self):
        email1 = fake.email()
        email2 = fake.email()
        token1 = VerifyEmail.get_token(email1)
        token2 = VerifyEmail.get_token(email2)
        email3 = VerifyEmail.verify_token(token2)
        email4 = VerifyEmail.verify_token(token1)
    ...
    def test_normalize(self):
        ...
        bigmail = username + '@' + domain.upper()

        token = VerifyEmail.get_token(bigmail)
        email1 = VerifyEmail.verify_token(token)
        self.assertEqual(email, email1)
    ...
    def test_grace_period(self):
        ...
        email2 = VerifyEmail.verify_token(token)
